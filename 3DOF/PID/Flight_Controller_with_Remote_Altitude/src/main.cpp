 ...
 * //////////////////////////////////////
 * UPDATED: July 2019 
 * UPDATED BY: Jaiden Evarts
 * 
 * All the altitude hold additions are labeled with the phrase "AltHold Add-In" so you can locate all the pieces more easily (each piece is bounded by *'s).
 * The altitude hold is turned on/off with the right control nob on the controller (you may have to switch around a few values if your nob assignments are flipped).  
 * The majority of the barometer code is well commented by Joop (I have a few lines in there that I added which I put my name after in the pertinent comment if you want 
 * to see exactly what I added).  
 * 
 * Good Luck!
 * ~Jaiden
 *  
...
//Variable for recognizing when new position data is available
int isData;
...
void read_barometer(void) {
  ...
  if (barometer_counter == 3) {                                                                               //When the barometer counter is 3
    ...
    if (altitudeHold && (activateHold > 1000)) {                                                          //If the quadcopter is in altitude mode and flying.
      ...
      if (isData){
        pid_altitude_setpoint += newZCoord;
      }
      //++++++++++++++++++++++++++++++++++++++
